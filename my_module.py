...
def draw_roc(y_test, probabilities, mode_05 = '', title = ''):
    ...
    for thr in thresholds:
        ...
        text_thr = f'<b>Порог: {round(thr, 4)}</b><br><br>TN: {tn}, FP: {fp}<br>FN: {fn}, TP: {tp}'
        text.append(text_thr)
    text[-1] = 'Порог: 0.0...<br><br>TN: 578, FP: 0<br>FN: 204, TP: 1'
    index_05 = abs(0.5 - thresholds).argmin()

    def return_layout(mode_05):
        layout = dict(xaxis = dict(range = (-0.025, 1)), yaxis = dict(range = (0, 1.05)), template = 'ggplot2',
                  ...
                  xaxis_title = dict(text ='fpr (1 - специфичность)', font = dict(size = 14.5)), 
        ...
        if mode_05 == 'explain':
            ...
            return layout
        layout.update(dict(annotations = [dict(text = annot,  x = fpr[index_05], y = tpr[index_05],
    ...
    data = [go.Scatter(x = fpr, y = tpr, text = text, mode = 'lines', fill = 'tozeroy', marker_color = 'green', 
            ...
            go.Scatter(x = [fpr[index_05]], y = [tpr[index_05]], mode = 'markers', 
                       hoverlabel = {'bgcolor': 'rgba(170, 170, 170, 0.3)'},
                       marker = dict(color = 'green', size = 10), text = [text[index_05]],
                       hovertemplate = '<extra></extra>%{text}<br>TPR: %{y:.3f}<br>FPR: %{x:.3f}')]

    fig = go.Figure(data = data, layout = return_layout(mode_05 = mode_05))
    fig.show(config = my_plotly_config())
#%%
#%%
#%%
#%%
