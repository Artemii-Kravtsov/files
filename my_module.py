...
def corr_matrix(df):
    ...
    fig, ax = plt.subplots(figsize = ([0.8 * df.shape[1] if df.shape[1] >= 9 else 7][0], 
                                      [0.4 * df.shape[1] if df.shape[1] >= 9 else 2.5][0]))
    sns.heatmap(corr_matrix.abs(), annot = corr_matrix, mask = corr_matrix_mask, cmap = 'Blues', cbar = False, ax = ax)
    ax.set_title('Корреляция между числовыми столбцами', y = 1.05, fontsize = 16)
